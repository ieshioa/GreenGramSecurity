...
public class JwtTokenProviderV2 {
    ...
    private String generateToken(UserDetails userDetails, long tokenValidMilliSecond) {
        return Jwts.builder()
                .claims(createClaims(userDetails))  // claims는 payload(내용)에 저장하고 싶은 내용을 저장
    ...
    private UserDetails getUserDetailsFromToken(String token) {
        try {       // jwt, json 둘다 문자열인데 jwt는 인증코드 json은 데이터
            ...
            return om.readValue(json, MyUserDetails.class); // json > 객체로 변환 (그것이 UserDetails 인데 정확히는 MyUserDetails이다)
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public String resolveToken(HttpServletRequest req) {
        ...
        if(!auth.startsWith(appProperties.getJwt().getTokenType())) { // Bearer 로 시작하지 않는다면 리턴널
            return null;
        }
